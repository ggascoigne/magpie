        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            GroupBox("Shortcuts & Quick Capture") {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text("Selected-text capture uses macOS Accessibility access. Magpie requests permission only when this option is enabled.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                .padding(6)
            ...
            GroupBox("Startup") {
                VStack(alignment: .leading, spacing: 6) {
                    Toggle(
                        "Launch Magpie at Login (Recommended)",
                        isOn: Binding(
                            get: { launchAtLogin.isEnabled },
                            set: { launchAtLogin.setEnabled($0) }
                        )
                    )
                    Text("Magpie lives in the menu bar, so launching it at login keeps Quick Capture available.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
